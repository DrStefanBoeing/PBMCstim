...
# If true, create viewer will trigger re-creation of the single-cell viewer.
# If only meta data that doesn't affect the viewer needs updating, it can be set to false.
# Example: Addition of new DGE comparisons.

createViewer <- FALSE

## Is the dataset to be publically accessible on the website?
publicDataset <- TRUE
...
if (file.exists(SeuratFN)){
    load(SeuratFN)
    print(paste0("Seurat object ", Obio@parameterList$localWorkDir,SeuratFN, " exists and is loaded."))
...
if (!exists("downsizeObject")){
    downsizeObject <- FALSE
}
...
pos <- grep("harmony.sketch", names(OsC@reductions))

if (length(pos) > 0 & downsizeObject){
    OsC <- subset(OsC, subset = cellID %in% row.names(Seurat::Embeddings(OsC@reductions$harmony.sketch)))
}
...
dfdbTable <- dfdbTable[,!duplicated(names(dfdbTable))]
...
```{r create_app_upload_tables, eval=createViewer, echo=F, results=F}
...
## Define helper function ##
perc_fun <- function(obj, genes, assay = "sketch") {
  ## This will ensure that 'odd' genes don't throw an error.

  # genes <- paste0("`", genes, "`")

  ## Get matrix
  mat <- obj@assays$RNA$data
  genes <- genes[genes %in% row.names(mat)]
  mat <- mat[genes, ]

  mat[mat > 0] <- 1

  ExprMatrix <- round(( rowSums(data.frame(mat)) / ncol(mat) ) * 100, 1 )
  dfExprMatrix <- data.frame(ExprMatrix)
  dfExprMatrix[["gene"]] <- row.names(dfExprMatrix)
  dfExprMatrix <- unique(dfExprMatrix)
  colnames( dfExprMatrix )[1] <- "count_cut_off"
  row.names(dfExprMatrix) <- dfExprMatrix$gene
  return(dfExprMatrix)
}
...
topDir <- unlist(strsplit(getwd(), "analyses\\/"))[1]
designDir <- paste0(topDir, "design/")

FNcolCustom <- paste0(
    designDir,
    "customClusterAnnotation.txt"
)


FNcol <- paste0(designDir, "clusterAnnotation.txt")
...
```{r, eval=createViewer, echo=F, results=F}
...
names(dfNormCounts) <- gsub("\\.", "_", names(dfNormCounts))

## Assemble database table ##
dfAnno  <- unique(dfAnno[dfAnno[, Obio@parameterList$geneIDcolumn] %in% unique(dfNormCounts$gene), ])

col <- Obio@parameterList$geneIDcolumn

database.table <- dplyr::full_join(
  dfAnno,
  dfNormCounts,
  by = dplyr::join_by({{col}} == "gene")
)
...
dflg2Avg <- dfNormCounts
row.names(dflg2Avg) <- dflg2Avg$gene
dflg2Avg$gene <- NULL
...
dflg2Avg <- dflg2Avg[apply(dflg2Avg, 1, mean) != 0, ]


dflg2Avg   <- t(apply(dflg2Avg,1,function(x){log2((x/mean(x)))}))


dflg2Avg[dflg2Avg == -Inf] <- 0
dflg2Avg <- data.frame(dflg2Avg)
...
dflg2Avg[["gene"]] <- row.names(dflg2Avg)
...
####################################################
## Create long-format list with expression values ##
# Note: It might be possible to speed this up with furr: https://stackoverflow.com/questions/41005938/how-to-make-purrr-map-function-run-faster
# This needs to be calculated for all cells.

# Testremoval next three lines.
# dfExpr <- purrr::map(cellList, function(x) subset_fun(obj=OsC, cellIDs = x)) %>%
#  dplyr::bind_rows() %>%
#  data.frame()
## Done                                           ##
...
perc_fun <- function(obj, genes, assay = "sketch") {
  ...
  if (assay == "sketch"){
    pos <- grep("sketch", names(obj@assays))
    if (length(pos) == 1){
      geneFilt <- genes[genes %in% row.names(obj@assays$sketch$data)]
      z <- subset(obj, features = geneFilt)

      z <- z@assays$sketch$data
    } else {
      z <- subset(obj, features = genes[genes %in% row.names(obj@assays$RNA$data)])
      z <- z@assays$RNA$data
    }
  }
...
## To speed up calculations, large datasets
# Note: It might be possible to speed this up with furr: https://stackoverflow.com/questions/41005938/how-to-make-purrr-map-function-run-faster
# This can be calculated on a subset of cells to speed things up.

OsC_CTF <- OsC
if (length(OsC_CTF@meta.data$cellID) > 20000){
  n.cells <- 20000
  cells.use = sample(x = OsC_CTF@meta.data$cellID, size = n.cells)
  OsC_CTF <- subset(x = OsC_CTF, subset = cellID %in% cells.use)
} else {
  OsC_CTF <- OsC
}


dfPercCellsExpr <- purrr::map(geneList, function(x) perc_fun(obj=OsC_CTF, genes = x)) %>%
  dplyr::bind_rows() %>%
  data.frame()

row.names(dfPercCellsExpr) <- NULL
dfPercCellsExpr <- unique(dfPercCellsExpr)

rm(OsC_CTF)
## Done                                            ##
#####################################################

dfExprMatrix <- dfPercCellsExpr


dfExprMatrix <- data.frame(dfExprMatrix)
...
biologicToolsSC::addClusterMarkers2CatDisplay(
  obj = Obio,
  addCorCatsToLabDb = T
)
...
names(dfAddCounts) <- paste0("add_counts_Avg_log10_Expr_", names(dfAddCounts) )
names(dfAddCounts) <- gsub( "add_counts_Avg_log10_Expr_gene", "gene", names(dfAddCounts))
names(dfAddCounts) <- gsub("\\.", "_", names(dfAddCounts))
...
#'
 ...
 #    selector4heatmap.cols = "Avg_log10_Expr_Sc"
...
for (i in 1:length(Obio@referenceTableList)) {
    df.ref <- biologicSeqTools2::import.db.table.from.db(
...
if (exists("df.ref") & nrow(df.ref) > 0){
    df.temp <- biologicSeqTools2::add2labCatSelectionDBtable(
        df.ref = df.ref,
...
if (!is.null(Obio@parameterList$clusterNameOrder)){
  sample.order <- paste0("norm_counts_Avg_log10_Expr_", Obio@parameterList$clusterNameOrder)
  if (length(sample.order) != length(grep("norm_counts_", names(Obio@databaseTable)))){
...
if (!is.null(Obio@parameterList$clusterNameOrder)){
  lg2Avg.order <- paste0("lg2_avg_Avg_log10_Expr_Sc_", Obio@parameterList$clusterNameOrder)
  if (length(lg2Avg.order) != length(grep("lg2_avg", names(Obio@databaseTable)))){
    lg2Avg.order <- (names(Obio@databaseTable)[grep("lg2_avg", names(Obio@databaseTable))])
  }
} else {
  lg2Avg.order <- (names(Obio@databaseTable)[grep("lg2_avg", names(Obio@databaseTable))])
}
