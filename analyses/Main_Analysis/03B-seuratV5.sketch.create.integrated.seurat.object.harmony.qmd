...
harmony.reg.vars <- c("meta_Batch", "meta_Run")

harmony.reg.vars <- harmony.reg.vars[harmony.reg.vars %in% names(OsC@meta.data)]

## Display Regression in report.
if (length(harmony.reg.vars) > 0){
    cat(paste0("Harmony regression columns: ", paste(harmony.reg.vars, collapse = ", "), "\n"))
}
...
OsC <- Seurat::IntegrateLayers(
      ...
      new.reduction = "harmony.sketch",
      verbose = FALSE,
      group.by.vars = harmony.reg.vars,  # variables to pass to harmony
      reduction.key = "Harmonysketch_"
    )
...
OsC <- Seurat::RunUMAP(
    ...
    return.model = TRUE,
    reduction.key = "UMAPharmonysketch_"
)
...
OsC <- Seurat::RunTSNE(
    ...
    dims = 1:Obio@parameterList$singleCellSeuratNpcs4PCA,
    reduction = "harmony.sketch",
    reduction.name = "tsne.harmony.sketch",
    return.model = TRUE,
...
OsC <- Seurat::FindNeighbors(OsC, reduction = "harmony.sketch", dims = 1:Obio@parameterList$singleCellSeuratNpcs4PCA)



# Find clusters using a range of resolutions
clusterOptionVec <- c(0.2,0.3,0.4, 0.5,0.7,0.9, 1,1,1.3,1.5, 1.7,1.9, 2.1,2.3,2.5)
OsC <- Seurat::FindClusters(object = OsC, reduction = "harmony.sketch", resolution = clusterOptionVec)
...
OsC <- Seurat::FindClusters(OsC, reduction = "harmony.sketch", resolution = Obio@parameterList$singleCellClusterParameter, cluster.name = "seurat_clusters_sketch")
...
OsC@reductions$harmony <- OsC@reductions$harmony.sketch.full
OsC@reductions$harmony.sketch.full <- NULL
OsC@reductions$harmony@key <- gsub("sketchfull","" ,OsC@reductions$harmony@key)
colIDs <- sapply(colnames(x = OsC@reductions$harmony@cell.embeddings), function(x) unlist(strsplit(x, "_"))[2])
nameVec <- paste0(
OsC@reductions$harmony@key,
colIDs
)
colnames(x = OsC@reductions$harmony@cell.embeddings) <- nameVec
...
## Adjusting names and labels
OsC@reductions$umap <- OsC@reductions$full.umap.harmony.sketch
OsC@reductions$full.umap.harmony.sketch <- NULL
