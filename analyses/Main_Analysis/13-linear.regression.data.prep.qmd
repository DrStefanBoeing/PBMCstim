...
legendDotSize <- 5
    ...
    #) + ggplot2::geom_abline(intercept = intersectVec[i], slope = slopeVec[i], linetype="dashed", color = "grey"
    ) + ggplot2::geom_smooth(method='lm', formula= y ~ x, linetype="dashed", se = T, colour = "grey"
    ) + ggrastr::rasterize(ggplot2::geom_point( shape=16, size = dotsize
            ), dpi = rasterDpi)  + ggplot2::xlab("Average Expression All Cells") + ggplot2::ylab(paste0("Average Expression ", clusterVec[[i]])
            ) + ggplot2::theme_bw(
            )  +  ggplot2::theme(
            ...
            ) + ggplot2::ggtitle(paste0("Cluster Markers  ", clusterVec[[i]])
            ) + ggplot2::scale_color_manual(values=c("#000000", clusterCol[i])
...
## Clusterresidual table might have changed - save to be safe.
