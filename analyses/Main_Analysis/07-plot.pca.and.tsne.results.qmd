...
plotList[[tag]] <- ggplot2::ggplot(data=dfPlot[dfPlot$included == "+",], ggplot2::aes(UMAP_1, UMAP_2, color=Cluster)
            ) + ggrastr::rasterize(
                    ggplot2::geom_point(
                        shape=16, size = as.numeric(dotsize)
                    ),
                    dpi = rasterDpi
...
if (h ==2){
    ...
    plotList[[tag]] <- plotList[[tag]] + ggplot2::scale_colour_manual("Clusters" ,values = colVec
    ) + ggplot2::guides(col =ggplot2::guide_legend(override.aes = list(shape = 16, size = legendDotSize))
    )
}
...
plotList[[tag]] <- ggplot2::ggplot(data=dfPlot[dfPlot$included == "+",], ggplot2::aes(tSNE_1, tSNE_2, color=Cluster)
            ) + ggrastr::rasterize(
...
for (i in 1:length(sampleVec)){
    ...
    plotList[[tag]] <- ggplot2::ggplot(data=dfPlotSel[dfPlotSel$included == "+",], ggplot2::aes(UMAP_1, UMAP_2, color=Cluster)
...
# plotList[[tag]] <- plotList[[tag]] + geom_point2(ggplot2::aes(subset=node==5), color='darkgreen', size=5)
